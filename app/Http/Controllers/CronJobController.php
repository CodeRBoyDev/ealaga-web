...
class CronJobController extends Controller
{
    ...
    public function updateAge()
    {
        try {
            ...
            $currentDate = Carbon::now(new \DateTimeZone('Asia/Manila'));

            // Get the users with birthdays from the database
            $birthdayUsers = DB::table('users')
            ...
            // Output the users whose birthday matches the current date
            $matchingBirthdayUsers = $birthdayUsers->filter(function ($user) use ($currentDate) {
                $birthday = Carbon::parse($user->birthday);
                return $birthday->format('md') === $currentDate->format('md');
            });
