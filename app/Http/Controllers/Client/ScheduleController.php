...
class ScheduleController extends Controller
{
    ...
    public function scheduleDateDisable(Request $request)
    {
        //
        try {
            ...
            $disable_holidays_date = DB::table('holidays')
            ->pluck('holidays.date')
            ->toArray();
            

                return response()->json(['services' => $services, 'schedule_total' => $scheduleTotals
                , 'selected_date_disable' => $selected_disable_dates
                , 'disable_holidays_date' => $disable_holidays_date]);
    ...
    public function ClientSchedule(Request $request)
    {
        ...
        try {
            ...
            if(auth()->user()->status != 1 || auth()->user()->is_restricted == 1){
    ...
    public function ClientScheduleHistory(Request $request)
    {
        ...
        try {
            ...
            ->where(function ($query) use ($currentDate) {
                $query->where('schedule.schedule_date', '<', $currentDate->format('Y-m-d'))
                    ->where('schedule.user_id', auth()->user()->id);
            })
            ->orWhere(function ($query) {
                $query->where('schedule.status', 3)
                    ->where('schedule.user_id', auth()->user()->id);
            })
